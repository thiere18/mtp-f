...
def get_continers(response: Response,db: Session = Depends(get_db),  current_user: int = Depends(oauth2.get_current_user)):
    return  container.get_containers(response, db)

@router.get("/search", response_model=List[schemas.ContainerOut])
def get_containers_search(db: Session = Depends(get_db), search: str=" ",  current_user: int = Depends(oauth2.get_current_user)):
