...
def get_clientss(response: Response,db: Session = Depends(get_db)):
    return  client.get_clientss(response,db)

@router.get("/search", response_model=List[schemas.ClientOut])
def search_client(db: Session = Depends(get_db), search: str = ""):
...
def update_client(id: int, updated_post: schemas.CategoryCreate, db: Session = Depends(get_db)):
