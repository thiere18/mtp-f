...
@router.get(
    "/", response_model=List[schemas.DepenseOut], response_model_exclude_none=True
)
def get_depenses(
    response: Response,
...
@router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=schemas.DepenseOut
)
def create_depense(
    post: schemas.Depensecreate,
    db: Session = Depends(get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
...
def update_depense(
    id: int,
    updated_post: schemas.CategoryCreate,
    db: Session = Depends(get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
