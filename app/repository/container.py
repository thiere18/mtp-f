...
def get_containers(response: Response, db: Session):
    containers = (
        db.query(models.Container).filter(models.Container.deleted != True).all()
    )
...
def get_container(id: int, db: Session):
    container = (
        db.query(models.Container)
        .filter(models.Container.id == id, models.Container.deleted != True)
        .first()
    )
    if not container:
        raise HTTPException(
...
def delete_container(id: int, db: Session):
    container_query = db.query(models.Container).filter(
        models.Container.id == id, models.Container.deleted != True
    )
...
def update_container(
    id: int, updated_post: schemas.CategoryCreate, db: Session = Depends(get_db)
):
    container_query = db.query(models.Container).filter(
        models.Container.id == id, models.Container.deleted != True
    )
