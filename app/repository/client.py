...
def search_client(db: Session, search: str):
    return (
        ...
        .filter(models.Client.deleted != True, models.Client.name.contains(search))
        .all()
    )
...
def get_client(id: int, db: Session = Depends(get_db)):
    client = (
        db.query(models.Client)
        .filter(models.Client.id == id, models.Client.deleted != True)
        .first()
    )
    if not client:
        raise HTTPException(
...
def delete_client(id: int, db: Session = Depends(get_db)):
    client_query = db.query(models.Client).filter(
        models.Client.id == id, models.Client.deleted != True
    )
