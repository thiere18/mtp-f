...
def get_magasin(id: int, db: Session):
    magasin = (
        ...
        .filter(models.Magasin.id == id, models.Magasin.deleted != True)
        .first()
    )
    if not magasin:
        raise HTTPException(
...
def delete_magasin(id: int, db: Session):
    magasin_query = db.query(models.Magasin).filter(
        models.Magasin.id == id, models.Magasin.deleted != True
    )
