...
def get_depots(db: Session, search: str = ""):
    return (
        ...
        .filter(models.Depot.deleted != True, models.Depot.name.contains(search))
        .all()
    )
...
def get_depot(id: int, db: Session):
    depot = (
        db.query(models.Depot)
        .filter(models.Depot.id == id, models.Depot.deleted != True)
        .first()
    )
    if not depot:
        raise HTTPException(
...
def delete_depot(id: int, db: Session):
    depot_query = db.query(models.Depot).filter(
        models.Depot.id == id, models.Depot.deleted != True
    )
